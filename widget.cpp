    ...
    , ui(new Ui::Widget)
{
    ...
    _imagecapture = new QImageCapture(_camera);
...
//    qDebug() << QThread::currentThreadId() << this;
    ...
    {
        QImage videoImg = QImage(frame.bits(1), frame.width(), frame.height(), QVideoFrameFormat::imageFormatFromPixelFormat(frame.pixelFormat()));
...
void Widget::datasolve(MESG *msg)
{
    ...
    else if(msg->msg_type == TEXT_RECV)
    {
        ...
        {
            QSoundEffect::play(":/myEffect/2.wav");
        }
    }
